...
addon:RegisterCondition("ECLIPSE", {
    description = "Eclipse",
    icon = "Interface\\Icons\\inv_potion_71",
    fields = { unit = "string", operator = "string", value = "number" },
    valid = function(_, value)
    ...
    evaluate = function(value, cache)
        if not addon.getCached(cache, UnitExists, value.unit) then return false end
        local cur = addon.getCached(cache, UnitPower, value.unit, 26)
        return addon.compare(value.operator, cur, value.value)
    end,
    print = function(_, value)
       return addon.compareString(value.operator, string.format(L["%s Eclipse"], addon.nullable(addon.unitsPossessive[value.unit], L["<unit>"])), addon.nullable(value.value))
    end,
    widget = function(parent, spec, value)
        local top = parent:GetUserData("top")
        local root = top:GetUserData("root")
        ...
        local operator_group = addon:Widget_OperatorWidget(value, L["Eclipse"],
            function() top:SetStatusText(funcs:print(root, spec)) end)
        parent:AddChild(operator_group)
    end,
    help = function(frame)
        addon.layout_condition_unitwidget_help(frame)
        frame:AddChild(helpers.Gap())
        addon.layout_condition_operatorwidget_help(frame, L["Eclipse"], L["Eclipse"],
            "The raw Eclipse value of " .. color.BLIZ_YELLOW .. L["Unit"] .. color.RESET .. ". " ..
            "If this number is negative, it means the Eclipse deficit (from max Eclipse).")
    end
})

addon:RegisterCondition("HOLYPOWER", {
    ...
    evaluate = function(value, cache)
        if not addon.getCached(cache, UnitExists, value.unit) then return false end
        local cur = addon.getCached(cache, UnitPower, value.unit, 9)
        return addon.compare(value.operator, cur, value.value)
    end,
    print = function(_, value)
       return addon.compareString(value.operator, string.format(L["%s HOLYPOWER"], addon.nullable(addon.unitsPossessive[value.unit], L["<unit>"])), addon.nullable(value.value))
    end,
    widget = function(parent, spec, value)
        local top = parent:GetUserData("top")
        local root = top:GetUserData("root")
        ...
        local operator_group = addon:Widget_OperatorWidget(value, L["HOLYPOWER"],
            function() top:SetStatusText(funcs:print(root, spec)) end)
        parent:AddChild(operator_group)
    end,
    help = function(frame)
        addon.layout_condition_unitwidget_help(frame)
        frame:AddChild(helpers.Gap())
        addon.layout_condition_operatorwidget_help(frame, L["HOLYPOWER"], L["HOLYPOWER"],
            "The raw Holypower value of " .. color.BLIZ_YELLOW .. L["Unit"] .. color.RESET .. ". " ..
            "If this number is negative, it means the Eclipse deficit (from max Eclipse).")
    end
})

addon:RegisterCondition("ENEMIES", {
    description = "Enemies",
    ...
    print = function(_, value)
       return addon.compareString(value.operator, "Near enemies", addon.nullable(value.value))
    end,
    widget = function(parent, spec, value)
        local top = parent:GetUserData("top")
        local root = top:GetUserData("root")
        ...
        local operator_group = addon:Widget_OperatorWidget(value, L["Enemies"],
            function() top:SetStatusText(funcs:print(root, spec)) end)
        parent:AddChild(operator_group)
    end,
    help = function(frame)
        addon.layout_condition_unitwidget_help(frame)
        frame:AddChild(helpers.Gap())
        addon.layout_condition_operatorwidget_help(frame, L["Enemies"], L["Enemies"],
            "")
    end
})

addon:RegisterCondition("ECLIPSEDIRECTION", {
    description = "Eclipse Direction",
    icon = "Interface\\Icons\\ability_hunter_pet_bear",
    fields = { value = "string" },
    valid = function(_, value)
    ...
    evaluate = function(value, cache)
        ...
        return GetEclipseDirection() == val
    end,
    print = function(_, value)
		local val = "none"
		if value.value == 1 then
