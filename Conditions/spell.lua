...
addon:RegisterCondition("SPELL_AVAIL", {
    ...
    evaluate = function(value, cache, evalStart)
        ...
        if start == 0 and duration == 0 and (usable or nomana) then
            return true
        else
            ...
            if gcd_start ~= 0 and gcd_duration ~= 0 then
                ...
                if (remain <= gcd_remain and (usable or nomana)) then
                    return true
                ...
                elseif (remain - gcd_remain <= time - evalStart and (usable or nomana)) then
                    return true
                else
                    return false
                end
